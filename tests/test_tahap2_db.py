"""
Unit Test untuk Tahap 2 - Database Connection, Entities, dan Repositories.

Modul ini menguji:
1. Class Entity: Course, RPS, BAP, ValidationResult, Report
2. Class DatabaseConnection (menggunakan mocking)
3. Kelas-kelas Repository (CourseRepository, RPSRepository, BAPRepository, ValidationRepository, UploadRepository) menggunakan mocking
...
class TestEntities:
    ...
    def test_course_entity(self):
        """Menguji inisialisasi, to_dict, dan from_dict dari entitas Course."""
        ...
        assert course.course_code == "IF101"
        assert course.course_name == "Pemrograman Objek"
        assert course.semester == "Ganjil"
        assert course.academic_year == "2024/2025"
        assert course.created_at == created_time

        # Test to_dict
        d = course.to_dict()
        assert d["course_code"] == "IF101"
        assert d["course_id"] == 1
        assert d["created_at"] == created_time

        # Test from_dict
        course_clone = Course.from_dict(d)
        assert course_clone.course_code == "IF101"
        assert course_clone.course_id == 1
        assert repr(course) == "Course(course_id=1, course_code='IF101', course_name='Pemrograman Objek', semester='Ganjil', academic_year='2024/2025')"
    ...
    def test_rps_entity(self):
        ...
        assert repr(rps) == "RPS(rps_id=10, course_id=1, meeting_number=1, topic='Pengenalan OOP')"
    ...
    def test_bap_entity(self):
        ...
        assert repr(bap) == "BAP(bap_id=20, course_id=1, meeting_number=1, meeting_date='2024-09-10')"
    ...
    def test_validation_result_entity(self):
        ...
        assert repr(vr) == "ValidationResult(validation_id=30, course_id=1, meeting_number=1, status='SESUAI', similarity_score=85.5)"
    ...
    def test_report_entity(self):
        ...
        assert report.course_name == "Pemrograman Objek"
        ...
        d = report.to_dict()
        assert d["course_name"] == "Pemrograman Objek"
        assert d["compliance_percentage"] == 92.3
        
        summary = report.get_summary()
        assert "Pemrograman Objek" in summary
        assert "92.30%" in summary
        assert repr(report) == "Report(course_id=1, course_name='Pemrograman Objek', compliance_percentage=92.30%)"
...
class TestDatabaseConnection:
    ...
    def test_execute_query_select(self, mock_pool_class):
        ...
        # Cursor dengan dictionary=True mengembalikan list of dict
        mock_cursor.description = [("col1", None, None, None, None, None, None)]
        mock_cursor.fetchall.return_value = [{"course_id": 1, "course_code": "IF101"}]
        ...
        results = db_conn.execute_query("SELECT * FROM courses WHERE course_id = %s", (1,))
        
        assert len(results) == 1
        assert results[0]["course_code"] == "IF101"
        mock_cursor.execute.assert_called_once_with("SELECT * FROM courses WHERE course_id = %s", (1,))
    ...
    def test_execute_non_query_insert(self, mock_pool_class):
        ...
        last_id = db_conn.execute_non_query("INSERT INTO courses ...", ("params",))
        assert last_id == 12
    ...
    def test_execute_transaction_success(self, mock_pool_class):
        ...
        queries = [
            ("INSERT INTO rps ...", (1, 1, "Topik 1")),
            ("INSERT INTO rps ...", (1, 2, "Topik 2"))
        ]
    ...
    def test_execute_transaction_rollback(self, mock_pool_class):
        ...
        # Lempar error pada pemanggilan execute kedua
        ...
        queries = [
            ("INSERT INTO rps ...", (1, 1, "Topik 1")),
            ("INSERT INTO rps INVALID...", (1, 2, "Topik 2"))
        ]
...
class TestRepositories:
    ...
    def test_course_repository(self):
        """Menguji CRUD di CourseRepository."""
        db_mock = MagicMock(spec=DatabaseConnection)
        repo = CourseRepository(db_mock)
        
        course = Course("IF202", "PBO", "Ganjil", "2024/2025")
        
        # Test Create
        db_mock.execute_non_query.return_value = 5
        course_id = repo.create(course)
        assert course_id == 5
        assert course.course_id == 5
        
        # Test Get by ID
        db_mock.execute_query.return_value = [{
            "course_id": 5, "course_code": "IF202", 
            "course_name": "PBO", "semester": "Ganjil", 
            "academic_year": "2024/2025", "created_at": None
        }]
        retrieved = repo.get_by_id(5)
        assert retrieved is not None
        assert retrieved.course_code == "IF202"
        
        # Test Get all
        db_mock.execute_query.return_value = [
            {"course_id": 5, "course_code": "IF202", "course_name": "PBO", "semester": "Ganjil", "academic_year": "2024/2025", "created_at": None},
            {"course_id": 6, "course_code": "IF303", "course_name": "Struktur Data", "semester": "Genap", "academic_year": "2024/2025", "created_at": None}
        ]
        all_courses = repo.get_all()
        assert len(all_courses) == 2
        assert all_courses[1].course_code == "IF303"
    ...
    def test_rps_repository(self):
        ...
        # Test Get by Course
        db_mock.execute_query.return_value = [
            {"rps_id": 1, "course_id": 1, "meeting_number": 1, "topic": "OOP Intro", "sub_topic": "", "cleaned_topic": "", "source_file": "", "created_at": None, "updated_at": None},
            {"rps_id": 2, "course_id": 1, "meeting_number": 2, "topic": "Inheritance", "sub_topic": "", "cleaned_topic": "", "source_file": "", "created_at": None, "updated_at": None}
        ]
        list_rps = repo.get_by_course(1)
        assert len(list_rps) == 2
        assert list_rps[0].topic == "OOP Intro"

        # Test delete by course
        repo.delete_by_course(1)
        db_mock.execute_non_query.assert_called_with(
    ...
    def test_bap_repository(self):
        ...
        bap = BAP(1, 1, "2024-09-10", "Belajar OOP Intro")
        ...
        # Test count by course
        db_mock.execute_query.return_value = [{"total": 14}]
        count = repo.count_by_course(1)
        assert count == 14
    ...
    def test_validation_repository(self):
        ...
        vr = ValidationResult(1, 1, 90.0, "SESUAI", rps_id=1, bap_id=1)
        ...
        # Test save_batch (menjalankan transaksi hapus dulu baru insert)
        results = [vr]
        db_mock.execute_transaction.return_value = True
        success = repo.save_batch(results, 1)
        assert success is True
    ...
    def test_upload_repository(self):
        ...
        upload_id = repo.create(1, "file.pdf", "/path/file.pdf", 250, "SUCCESS")
        assert upload_id == 7
        
        # Test get_by_course
        db_mock.execute_query.return_value = [
            {"upload_id": 7, "course_id": 1, "file_name": "file.pdf", "file_path": "/path/file.pdf", "file_size_kb": 250, "upload_status": "SUCCESS", "error_message": None, "uploaded_at": None}
        ]
        history = repo.get_by_course(1)
